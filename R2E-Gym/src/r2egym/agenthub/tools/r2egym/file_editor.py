...
class StrReplaceEditor:
    ...
    def view(
        ...
        if path.suffix not in [".py",".rst"] and python_only:
            error_msg = (
                f"ERROR: Viewing non-Python files is disallowed for saving context. "
                f"File '{path.name}' is not a .py or .rst file."
            )
        ...
        if path.suffix in [".py",".rst"] and not view_range and not concise:
            file_text_tmp = self.read_path(path)
            if len(file_text_tmp.splitlines()) > 110:
                concise = True

        # For a file
        if path.suffix in [".py",".rst"] and concise:
    ...
    def str_replace(self, path: Path, old_str: str, new_str: str) -> EditorResult:
        ...
        if self.enable_linting and path.suffix in [".py",".rst"]:
